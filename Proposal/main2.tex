...
By providing real-time monitoring of cattle location and activity patterns, the system enables farmers to quickly identify animals in distress, detect health issues early, and respond promptly to emergency situations.
...
SMS & Short Message Service \\ \hline
...
\item Identify unusual behavior patterns indicating health issues
...
\item \textbf{Age Range:} 30-60 years
...
\item Generate reports for business purposes
\end{itemize}
...
\item \textbf{Age Range:} 20-50 years
...
\item \textbf{Age Range:} 25-45 years
...
Farm Owner/Admin & Basic-Intermediate & Mobile + Desktop & Easy interface, comprehensive data \\ \hline
...
\begin{itemize}
\item Android devices: Android 10.0+ required
\item Minimum screen size: 4.7 inches
\item Cellular or WiFi connectivity required
\item Minimum 2GB RAM, 32GB storage
\end{itemize}

\subsubsection{Desktop Computers}
\begin{itemize}
\item Windows 10+ or macOS 10.15+
\item Modern web browser (Chrome, Firefox, Safari, Edge)
\item Minimum 4GB RAM
\item Internet connection: 5 Mbps minimum
\end{itemize}
...
\begin{itemize}
\item Native Android application (Kotlin)
\item Minimum API level: Android API 29
\end{itemize}
...
\begin{itemize}
\item \textbf{Cellular:} 4G coverage in grazing areas
\item \textbf{Internet:} Broadband minimum 10 Mbps at farm office
...
The system shall authenticate users by verifying username/email and password credentials against the encrypted database. Upon successful authentication, the system shall create a secure session token valid for 24 hours.
...
The system shall provide password reset functionality via email verification. Users shall receive a time-limited reset link (valid 1 hour) to set a new password.

\paragraph{FR-05: Role-Based Access Control}
The system shall implement two primary user roles:
\begin{itemize}
\item \textbf{Administrator:} Full system access including user management and system configuration
\item \textbf{Viewer:} Read-only access to reports and dashboards
\end{itemize}

\paragraph{FR-06: Session Management}
The system shall automatically terminate inactive user sessions after 30 minutes. Users shall receive a warning 5 minutes before automatic logout.

\subsection{Cattle Registration and Management}

\paragraph{FR-07: Add New Cattle Record}
...
\paragraph{FR-08: Update Cattle Information}
...
\paragraph{FR-09: Remove Cattle Record}
...
\paragraph{FR-10: Search and Filter Cattle}
...
\paragraph{FR-11: Create Cattle Groups}
The system shall allow administrators to define cattle groups (e.g., "Breeding Stock," "Calves 2024") and assign multiple cattle to each group.

\subsection{Real-Time Location Tracking}

\paragraph{FR-12: Receive GPS Coordinates}
...
\paragraph{FR-13: Data Transmission Frequency}
...
\paragraph{FR-14: Display Current Locations}
...
\paragraph{FR-15: Map Interaction Features}
The map interface shall support zoom, pan, marker click for details, legend, and layer controls for geofences.

\paragraph{FR-16: Location Refresh}
...
\paragraph{FR-17: Individual Cattle Tracking}
The system shall allow users to select a specific animal and view its current location, recent movement path (last 24 hours), and relevant details.

\paragraph{FR-18: Store Location History}
...
\paragraph{FR-19: Display Historical Paths}
...
\paragraph{FR-20: Record Movement Data}
...
\paragraph{FR-21: Activity Classification}
The system shall classify cattle activity into categories: Active (moving/grazing), Resting (lying down), Feeding (head-down position), and Unknown.

\paragraph{FR-22: Calculate Daily Activity Metrics}
The system shall compute daily activity summaries for each animal: total active time, total rest time, activity percentage, and number of transitions.

\paragraph{FR-23: Activity Pattern Recognition}
...
\paragraph{FR-24: Comparative Activity Analysis}
...
\paragraph{FR-25: Create Geofence Areas}
...
\paragraph{FR-28: Detect Geofence Violations}
...
\paragraph{FR-29: Geofence Status Display}
...
\paragraph{FR-30: Geofence Violation Alerts}
...
\paragraph{FR-31: Inactivity Alerts}
The system shall generate medium-priority alerts when cattle activity falls below threshold levels for extended periods (configurable, default 4 hours).

\paragraph{FR-32: Device Malfunction Alerts}
The system shall generate alerts when collar device battery falls below 15%, GPS signal is lost for more than 30 minutes, or device communication failure occurs.

\paragraph{FR-33: Unusual Activity Alerts}
...
\paragraph{FR-34: Push Notifications}
...
\paragraph{FR-35: SMS Notification}
The system shall support optional SMS text message delivery of critical alerts to designated phone numbers.

\paragraph{FR-36: In-App Alert Center}
The mobile application shall maintain an Alert Center displaying all alerts with status (new, acknowledged, resolved).

\paragraph{FR-37: Acknowledge Alerts}
...
\paragraph{FR-40: Real-Time Dashboard}
...
\paragraph{FR-41: Individual Cattle Dashboard}
...
\paragraph{FR-42: Herd Overview Analytics}
The system shall provide herd-level analytics including activity distribution charts, location heat maps, health status summary, and trend indicators.

\paragraph{FR-43: Power BI Integration}
...
\paragraph{FR-44: Daily Activity Reports}
...
\paragraph{FR-45: Custom Report Builder}
The system shall provide a report builder allowing users to create custom reports by selecting data sources, date ranges, and output format.

\paragraph{FR-46: Scheduled Reports}
The system shall allow administrators to schedule automated report generation and delivery via email at specified intervals (daily, weekly, monthly).

\paragraph{FR-47: Trend Analysis}
...
\paragraph{FR-48: Comparative Analysis}
The system shall allow comparison of metrics between individual cattle, groups, and time periods to identify outliers and patterns.

\subsection{System Administration}

\paragraph{FR-49: User Account Management}
The system shall allow administrators to create new accounts, modify user details, change roles, reset passwords, and deactivate accounts. All actions shall be logged.

\paragraph{FR-50: Audit Logging}
The system shall maintain comprehensive audit logs recording user actions, system changes, data modifications, and security events with timestamps.

\paragraph{FR-51: General System Settings}
...
\paragraph{FR-52: Device Management}
...
\paragraph{FR-53: Notification Configuration}
The system shall allow configuration of alert thresholds, notification channels, escalation rules, and quiet hours.

\paragraph{FR-54: Data Backup}
...
The system shall support at least 50 concurrent users accessing the mobile or web applications simultaneously without performance degradation.
...
The system shall maintain 99.5\% uptime excluding planned maintenance windows, allowing maximum 43.8 hours of downtime per year.

\paragraph{NFR-10: Data Recovery Time}
In the event of system failure, data recovery and system restoration shall be completed within 4 hours.
...
\paragraph{NFR-13: Fault Tolerance}
The system shall continue operating with degraded functionality if individual components fail, ensuring critical features remain available.

\subsection{Security Requirements}

\paragraph{NFR-14: Data Encryption}
All sensitive data shall be encrypted both in transit (using TLS 1.2+) and at rest (using AES-256 encryption).

\paragraph{NFR-15: Password Storage}
User passwords shall be hashed using bcrypt algorithm with salt before storage. Plain text passwords shall never be stored.

\paragraph{NFR-16: Session Security}
User sessions shall be secured with randomly generated tokens, automatic timeout after 30 minutes of inactivity, and secure cookie attributes.

\paragraph{NFR-17: Access Control}
...
\paragraph{NFR-18: Audit Trail}
All security-relevant events (login attempts, permission changes, data modifications) shall be logged with timestamp, user identity, and action details.

\paragraph{NFR-19: Data Privacy}
...
\paragraph{NFR-20: Learning Curve}
...
\paragraph{NFR-27: Version Control}
All source code shall be maintained in Git version control system with meaningful commit messages.

\paragraph{NFR-28: Database Maintenance}
The system shall support routine maintenance operations (backup, optimization, archival) without requiring system shutdown.

\subsection{Scalability Requirements}

\paragraph{NFR-29: Cattle Population Support}
The system shall support monitoring of at least 5,000 individual cattle per installation without hardware upgrades.

\paragraph{NFR-30: Horizontal Scalability}
...
\paragraph{NFR-31: Data Storage Capacity}
...
\paragraph{NFR-32: Android Platform Support}
The mobile application shall run on Android devices version 10.0 and above without modification.

\paragraph{NFR-33: Cloud Platform Independence}
...
\paragraph{NFR-34: Planned Maintenance}
Planned maintenance windows shall be scheduled during low-usage periods (midnight to 4 AM) with 48-hour advance notification.

\paragraph{NFR-35: Offline Capability}
...
The system shall provide native mobile application for Android (version 10.0 and above).
...
The system shall communicate with GPS collar devices using MQTT protocol over secure TLS connection.
...
\paragraph{IR-09: Device Command Interface}
The system shall support sending configuration commands to devices including data transmission frequency adjustment and low-power mode activation.

\paragraph{IR-10: Mobile Device GPS Integration}
...
\paragraph{IR-11: Cloud IoT Service}
The system shall interface with Azure IoT Hub or AWS IoT Core using standard MQTT or AMQP protocols.

\paragraph{IR-12: Cloud Database Interface}
The system shall interface with cloud database services using standard SQL queries and ORM frameworks.

\paragraph{IR-13: Cloud Storage Interface}
The system shall interface with cloud blob storage for storing images, reports, and backup files.

\paragraph{IR-14: Mapping Service Integration}
The system shall integrate with Google Maps API or equivalent for map display and geocoding functions.

\paragraph{IR-15: SMS Gateway Interface}
The system shall interface with SMS gateway service via REST API for sending text notifications.

\paragraph{IR-16: Email Service Interface}
The system shall interface with email delivery service via SMTP or REST API for notifications and reports.

\paragraph{IR-17: Power BI Integration}
...
\paragraph{IR-18: HTTPS Communication}
All communication between client applications and cloud services shall use HTTPS with TLS 1.2 or higher encryption.

\paragraph{IR-19: RESTful API}
The system shall expose RESTful API following standard HTTP methods (GET, POST, PUT, DELETE).

\paragraph{IR-20: JSON Data Format}
All API communications shall use JSON format for request and response bodies.

\paragraph{IR-21: API Authentication}
API requests shall be authenticated using bearer tokens (JWT) transmitted in Authorization headers.

\paragraph{IR-22: API Rate Limiting}
The API shall implement rate limiting (default 1000 requests per hour) with appropriate HTTP 429 status codes.
...
\subsection{Use Case Diagram} \begin{figure}[H] \centering \includegraphics[width=0.8\textwidth]{UseCase.jpg} \caption{Use Case Diagram of CattleCare Pro} \end{figure}
...
\textbf{Geofencing:} Use of GPS or RFID technology to create virtual geographic boundary, enabling software to trigger response when device enters or leaves area.
...
\begin{longtable}{|p{2cm}|p{5cm}|p{2.5cm}|p{3.5cm}|}
\caption{Requirements Traceability} \\
\hline
\textbf{Req ID} & \textbf{Requirement} & \textbf{Priority} & \textbf{Test Case} \\ \hline
\endfirsthead
\hline
\textbf{Req ID} & \textbf{Requirement} & \textbf{Priority} & \textbf{Test Case} \\ \hline
\endhead
FR-01 & User account creation & High & TC-AUTH-01 \\ \hline
FR-02 & Secure login & High & TC-AUTH-02 \\ \hline
FR-03 & Password security & High & TC-AUTH-03 \\ \hline
FR-12 & Receive GPS coordinates & Critical & TC-GPS-01 \\ \hline
FR-15 & Display current locations & Critical & TC-MAP-01 \\ \hline
FR-27 & Create geofence areas & High & TC-GEO-01 \\ \hline
FR-32 & Geofence violation alerts & Critical & TC-ALERT-01 \\ \hline
FR-42 & Real-time dashboard & High & TC-DASH-01 \\ \hline
FR-45 & Power BI integration & Medium & TC-BI-01 \\ \hline
NFR-01 & UI response time & High & TC-PERF-01 \\ \hline
NFR-09 & System uptime & High & TC-REL-01 \\ \hline
NFR-14 & Data encryption & Critical & TC-SEC-01 \\ \hline
\end{longtable}
